...
| `get_admin() -> Result<Address, Error>` | Returns the configured admin. Returns `NotInitialized` before `initialize`. |
...
| `get_admin() -> Result<Address, Error>` | Returns the configured admin. Returns `NotInitialized` before `initialize`. |
| `get_attester_registry() -> Result<Address, Error>` | Returns the configured `attester-registry` contract. Returns `NotInitialized` before `initialize`. |
